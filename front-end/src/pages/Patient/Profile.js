...
const PatientProfile = () => {
  ...
  const fetchPregnancyInfo = async (patientId) => {
    try {
      ...
      const filteredPregnancy = pregnancies.find(p => p.patientId === patientId);
      if (filteredPregnancy) {
        setPregnancyInfo(filteredPregnancy);
      } else {
  ...
  return (
    <div className="container mx-auto p-6" style={{ backgroundColor: '#F4F7FB' }}>
      <h2 className="text-center text-3xl font-semibold text-gray-800 mb-6">Patient Profile</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {pregnancyInfo && (
          <div className="bg-white shadow-lg rounded-lg p-6 transition-all hover:shadow-2xl border border-gray-200">
            <h2 className="text-2xl font-semibold text-gray-800 mb-6">Pregnancy Information</h2>
            <div className="space-y-4">
              <div className="bg-indigo-50 p-4 rounded-lg shadow-sm">
                <strong className="text-lg text-indigo-700">Month of Pregnancy</strong>
                <p className="text-sm text-gray-600">{pregnancyInfo.monthOfPregnancy}</p>
              </div>
              <div className="bg-indigo-50 p-4 rounded-lg shadow-sm">
                <strong className="text-lg text-indigo-700">City</strong>
                <p className="text-sm text-gray-600">{pregnancyInfo.city}</p>
              </div>
              <div className="bg-indigo-50 p-4 rounded-lg shadow-sm">
                <strong className="text-lg text-indigo-700">Hospital</strong>
                <p className="text-sm text-gray-600">{pregnancyInfo.hospitalName}</p>
              </div>
              <div className="bg-indigo-50 p-4 rounded-lg shadow-sm">
                <strong className="text-lg text-indigo-700">Doctor</strong>
                <p className="text-sm text-gray-600">{pregnancyInfo.doctorName}</p>
              </div>
            </div>
          </div>
        )}
